...
const LeadDetailModal = ({ isOpen, onClose, lead, token }) => {
  ...
  const fetchActivities = async () => {
    try {
      const res = await axios.get(`${API_URL}/activities/lead/${lead._id}`, {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-2 sm:p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-5xl rounded-[2rem] sm:rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[96vh] sm:max-h-[92vh] border border-white/20">
        {/* Modern Glass Header */}
        <div className="px-6 py-6 sm:px-10 sm:py-8 bg-slate-900 relative overflow-hidden shrink-0">
          ...
          <div className="relative flex items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4 sm:gap-6 text-white min-w-0">
              <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br from-red-600 to-red-700 rounded-xl sm:rounded-2xl flex items-center justify-center font-black text-xl sm:text-2xl shadow-2xl shadow-red-900/20 ring-4 ring-white/10 shrink-0">
                {lead.business_name ? lead.business_name[0] : 'V'}
              </div>
              <div className="min-w-0">
                <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 mb-1">
                  <h2 className="text-lg sm:text-2xl font-black uppercase tracking-tight truncate">{lead.business_name}</h2>
                  <span className="inline-block w-fit px-2 py-0.5 sm:px-3 sm:py-1 bg-white/10 rounded-full text-[8px] sm:text-[10px] font-black uppercase tracking-widest border border-white/10">
                    {lead.lead_status}
                  </span>
                </div>
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-slate-400">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest">Enterprise Intelligence</span>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-10">
            ...
            <div className="lg:col-span-4 space-y-6 sm:space-y-8">
              ...
              <section className="bg-white p-6 sm:p-8 rounded-[1.5rem] sm:rounded-[2.5rem] shadow-sm border border-slate-100 space-y-4 sm:space-y-6">
                <div className="flex items-center gap-2 mb-2">
                  <div className="h-1 w-6 bg-red-600 rounded-full" />
                  <h3 className="text-[8px] sm:text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Pipeline Meta</h3>
                </div>
                <div className="space-y-3 sm:space-y-4">
                  {[
                    { label: 'Ingestion Date', value: new Date(lead.created_at).toLocaleDateString(), icon: Calendar },
                    { label: 'Origin Source', value: lead.lead_source, icon: TrendingUp, color: 'text-red-600' },
                    { label: 'Account Owner', value: lead.assigned_user?.name || 'Unassigned', icon: Briefcase },
                  ].map((item, i) => (
                    <div key={i} className="flex items-center justify-between p-2.5 sm:p-3 bg-slate-50 rounded-xl sm:rounded-2xl">
                      ...
                      <span className={cn("text-[10px] sm:text-xs font-black", item.color || "text-slate-700")}>{item.value}</span>
                    </div>
                  ))}
                </div>
              </section>
