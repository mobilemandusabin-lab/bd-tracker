...
const VendorModal = ({ isOpen, onClose, onSuccess, token }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    const loadingToast = toast.loading('Creating new vendor profile...');
    try {
      await axios.post(`${API_URL}/leads`, { ...formData, type: 'vendor' }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        <div className="px-8 py-6 bg-red-600 flex items-center justify-between">
          <div className="flex items-center gap-3 text-white">
            <UserPlus size={24} />
            <h2 className="text-xl font-black uppercase tracking-widest">Add New Vendor</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input
                name="business_name" required onChange={handleChange}
            ...
            <div className="space-y-2">
              ...
              <input
                name="phone" required onChange={handleChange} maxLength={10} pattern="\d{10}"
                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-red-600 outline-none font-bold text-sm"
