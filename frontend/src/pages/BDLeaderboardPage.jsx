...
const BadgeIcon = ({ badge, size = 24 }) => {
  ...
  return badgeMap[badge] || <span className="text-2xl">{badge}</span>;
};
    ...
    <div className="min-h-screen bg-slate-50 p-4 lg:p-8">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-4 mb-6">
          <Link 
            to="/dashboard" 
            className="p-2 bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow text-slate-600"
          >
            <ArrowLeft size={20} />
          </Link>
          <div>
                ...
                <div className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="text-lg font-black text-slate-900">{item.user?.name || 'Unknown'}</h3>
                    ...
                    <div className="text-right">
                      ...
                      <div className="flex gap-1 mt-1 justify-end">
                        {item.badges?.map((badge, idx) => (
                          ...
                          >
                            <BadgeIcon badge={badge.emoji} size={20} />
                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-slate-900 text-white text-[8px] font-bold rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                              {badge.title}
                            </div>
                          </div>
                        ))}
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                    <div className="bg-slate-50 rounded-xl p-3">
                      ...
                      <p className="text-lg font-black text-slate-900">{item.total_assigned || 0}</p>
                    </div>
                    <div className="bg-green-50 rounded-xl p-3">
                      <p className="text-[8px] font-black text-green-600 uppercase tracking-widest mb-1">Converted</p>
                      <p className="text-lg font-black text-green-700">{item.converted || 0}</p>
                    </div>
                    <div className="bg-blue-50 rounded-xl p-3">
                      <p className="text-[8px] font-black text-blue-600 uppercase tracking-widest mb-1">Conv. Rate</p>
                      <p className="text-lg font-black text-blue-700">{item.conversion_rate?.toFixed(1) || 0}%</p>
                    </div>
                    <div className="bg-purple-50 rounded-xl p-3">
                      <p className="text-[8px] font-black text-purple-600 uppercase tracking-widest mb-1">Avg. Days</p>
                      <p className="text-lg font-black text-purple-700">{item.avg_conversion_days?.toFixed(1) || 'N/A'}</p>
                    </div>
                  </div>
      ...
      {/* Legend */}
      <div className="max-w-7xl mx-auto mt-8 bg-white rounded-2xl shadow-sm p-6">
        <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Badge Legend</h3>
